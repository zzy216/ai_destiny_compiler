...
- [ ] 系统模型和用户自定义模型支持创建、列表、详情、更新草稿和软删除。
- [ ] 模型支持草稿版本发布、停用和系统默认模型切换。
- [ ] 模型连接测试继续使用草稿优先、已发布版本兜底的规则。
- [ ] API 响应不包含明文 API Key、密文或加密元数据。
- [ ] 新增行为有测试，类型检查和覆盖率检查通过。
- [ ] MVP 执行清单和 TDD 证据记录已更新。
