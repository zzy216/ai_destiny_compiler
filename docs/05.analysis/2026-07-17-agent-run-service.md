...
- [ ] 同一用户和幂等键只创建一条 Agent Run，重试返回已有记录。
- [ ] 创建记录默认状态为 `running`，并保存必需关联字段和快照。
- [ ] 成功、失败、超时和取消均写入合法终态及完成时间。
- [ ] 已结束运行不能再次更新；Token、成本和耗时不能为负数。
- [ ] 模型/教练快照及错误信息不得写入敏感凭据、Prompt、聊天正文或堆栈。
- [ ] 相关测试、类型检查和覆盖率检查通过，并更新阶段清单与 TDD 证据。
