...
export function ModelsPage() {
  ...
  const createMutation = useMutation({ mutationFn: (input: CreateModelInput) => apiClient.createModel(input), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['admin', 'models'] }); setOpen(false); form.reset(); message.success('模型草稿已创建'); } });
  ...
  return <div className="page-stack">
    <div className="page-heading"><div><h2>模型设置</h2><p>维护系统模型、发布版本和连接状态；凭据只展示脱敏信息。</p></div><Button type="primary" onClick={() => setOpen(true)}>新增模型</Button></div>
    <Card>{models.length === 0 ? <EmptyState description="还没有系统模型" /> : <Table rowKey="id" dataSource={models} pagination={false} columns={[
      ...
      { title: '操作', render: (_: unknown, record: Model) => <Space wrap><Button size="small" onClick={() => void action.mutateAsync({ id: record.id, kind: 'test' })}>连接测试</Button>{record.status === 'draft' && <Button size="small" type="primary" onClick={() => void action.mutateAsync({ id: record.id, kind: 'publish' })}>发布</Button>}{record.status === 'published' && <Button size="small" onClick={() => void action.mutateAsync({ id: record.id, kind: 'disable' })}>停用</Button>}{record.status === 'published' && !record.isDefault && <Button size="small" onClick={() => void action.mutateAsync({ id: record.id, kind: 'default' })}>设为默认</Button>}</Space> },
    ]} />}</Card>
    <Modal title="新增系统模型" open={open} onCancel={() => setOpen(false)} onOk={() => void form.handleSubmit((values) => createMutation.mutate(values as CreateModelInput))()} confirmLoading={createMutation.isPending}>
